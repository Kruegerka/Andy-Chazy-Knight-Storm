...
void setup() {
  ...
  //sendToWifi("AT+IPR=115200",responseTime,DEBUG);

  sendToWifi("AT",responseTime,DEBUG); // check connection
  ...
  if(wifiSerial.available()){
    ...
    if(find(message,"esp8266:")){
      ...
      if(find(result,"OK"))
        ...
        sendData("\nStatement Fail");               //At command ERROR CODE for Failed Executing statement
    }else
    if(find(message,"HELLO")){  //receives HELLO from wifi
        sendData("HI!");    //arduino says HI
      ...
      //digitalWrite(13,LOW);
        startTimer(0); //Turns off the timer and LED
